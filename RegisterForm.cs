...
namespace VotingSystemWinForms
{
    public partial class RegisterForm : Form
    {
        ...
        private void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(username) || string.IsNullOrEmpty(fullName) || string.IsNullOrEmpty(id))
            {
                MessageBox.Show("Please fill in all fields.", "Registration Failed", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (RegisterUser(username, fullName, id))
            {
            ...
            {
                MessageBox.Show("Registration failed. Username or ID may already exist.", "Registration Failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private bool RegisterUser(string username, string fullName, string id)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    connection.Open();
                    string sql = "INSERT INTO Users (Username, FullName, ID) VALUES (@Username, @FullName, @ID)";
